...
import PersonSelect from '../UI/selects/personSelect/PersonSelect'
...
// Значения для инпутов
  ...
  const {error, token, message} = useSelector(state => state.case);
  const [tok, setTokr] = useState(token)
  ...
  const handleMessage = (e) => {
    ...
    message.color = col
    message.date = dat
    message.officer = offic
    message.description = descr
    fetchAddCases(message,  'https://sf-final-project-be.herokuapp.com/api/cases/', localStorage.getItem('myToken'))
    if(error === null) {
      num.onChange('')
      fullname.onChange('')
      typ.onChange('')
